            ...
            <div class="card-body table-responsive">
                ...
                <table class="table table-sm table-hover">
                    ...
                    @foreach($apiLogList as $k => $apiLog)
                        <tr>
                            <td>{{$k+1}}</td>
                            <td>【{{get_chinese_weekday($apiLog->date)}}】{{$apiLog->date}}</td>
                            <td>{{$apiLog->count}}</td>
                            <td>{{intval($apiLog->avg)}}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
                <div class="float-right">
                    {{$apiLogList->withPath(jump_link('/hm/log-viewer/seven-visits-item'))->appends(request()->query())->links('pagination::bootstrap-4')}}
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    $(document).on("click",".api-log-search",function(){
        ...
        var url = jump_link('/hm/log-viewer/seven-visits-item?') + formData; // 拼接URL
        // 跳转
